		...
		if (!empty($postStr)){
            ...
            if ($recMsgType == "text"){							//接收到用户的文本消息
				...
				if(!empty( $keyword ) || $keyword == "0")			
                {
					 ...
					 *				保存用户命令记录
					 */////////////////////////////////////////////////
					insert_wechat_user_instruct($db,$fromUsername,$keyword);
					/*/////////////////////////////////////////////////
					 *						END
					 *////////////////////////////////////////////////
					if ($action == "wechat_main"){
						if ($keyword == "返回蹭课" || $keyword == "1"){
							...
							$code = "20";
						}else if ($keyword == "号码" || $keyword == "号码通" || $keyword == "10"){
							$code = "61";						
						}else if ($keyword == "旧版本" || $keyword == "旧版" || $keyword == "11"){
							$code = "1000";
						}else{
							$code = "0";
						}
					 ...
					 */
						...
						if ($keyword == "返回蹭课" || $keyword == "1"){
							...
							$null_code = activity_apply_submit_handle($db,$fromUsername,$activity_apply_list);//返回第一个不符合条件的空值
						...
						setTempDataWithIndex($db, $fromUsername, $index, $data);	//存入临时表中
					 ...
					 */
						...
						setTempDataWithIndex($db, $fromUsername, $myindex, $student_name);			//将学号存入临时数据表
						...
						$student_no = getTempDataWithIndex($db,$fromUsername, $index);				//获取临时表中的学号
						
						//在数据库中找出学号对应的姓名,如果匹配成功，绑定学号跳到成功页，否则，跳到失败页
						if ($keyword == getNameWithStudentNo($db,$student_no)){
					 ...
					 */
						...
						if (($content = get_lesson_with_name($db,$keyword)) !== false){
							$code = "33";	
						}else {												//查看成功
							$code = "34";
						}
						...
						if (($content = get_lesson_with_time($db,$keyword)) !== false){
							$code = "33";														//查看成功
						}else {
							$code = "34";
						}
					 ...
					 */
						...
						if ($keyword == "返回蹭课" || $keyword == "1"){
							...
							$cur_page = 1;
							$picText_list = get_room_record($db,$fromUsername,$cur_page,"room_record_cur_page");		//里面有自动插入页数到临时表的机制
							
							$code = "45";
						}else if ($keyword == "退出房间" || $keyword == "退出" || $keyword == "4"){
							//$content = get_my_room_record($db,$fromUsername);
							$code = "48";
						}else {
							$code = "40";
						}
						...
						if ($keyword == "返回蹭课" || $keyword == "1"){
							...
							$cur_page = getTempDataWithIndex($db,$fromUsername,"room_record_cur_page")-1;
							$code = "45";
							$picText_list = get_room_record($db,$fromUsername,$cur_page,"room_record_cur_page");
						...
						setTempDataWithIndex($db, $fromUsername, $index, $room_name);				//存入临时表中
						...
						$room_num = $keyword;
						if (($bool=delete_room_with_orderNum($db,$fromUsername, $room_num)) === false){		//删除出错
							$code = "47";
						}else {			//成功删除
							$code = "49";
						}
					...
					/*else if ($action == "express_submit_success"){
					...
					else if ($action == "telephone_ask"){
						$phone_name = mb_convert_encoding($keyword, "gbk", "UTF-8");
						/*获取电话信息*/
						if ($result = getTelPhoneWithName($db, $phone_name)){
							$count = count($result);
							$content = "";
							$mytext = "%s\n%s %s\n";
							for ($i=0;$i<$count;$i++){
								$content .= sprintf($mytext,$result[$i]['name'],$result[$i]['addr'],
									$result[$i]['phone']); 
							}
						}else{
							$content = "未知联系人";
						...
						$code = "62";
						$wechat_user_db->set_interact_code($fromUsername,$code);	//查找成功
					}
					/*
					 * 其他
					 */
					...
					if ($action == "wechat_main"){			
						...
						$content = WINDOWS::$type['wechat_main_pictxt'];
						$sender_type = "content_picText";
					 ...
					 */
						...
						$picText_list = getActivityList($db);
						...
						$sender_type = "picText";
						$picText_list = getDoubanActivity($db);
					 ...
					 */
						...
						$content = sprintf($content, URL::$LOCAL, "show_lesson" , getStudentNoWithOpenId($db,$fromUsername));					//传递GET类型show_lesson,学号
					 ...
					 */
						...
						$content = WINDOWS::$type['telephone_ask'];
					}
					else if ($action == "telephone_answer_success"){
						$content = $content.WINDOWS::$type['wechat_main'];
						$code = "0";
						$wechat_user_db->set_interact_code($fromUsername,$code);//显示当前活动主页面，并监听
					}
					else if ($action == "telephone_answer_fail"){
						$content = WINDOWS::$type['telephone_answer_fail'].WINDOWS::$type['wechat_main'];
						$code = "0";
						$wechat_user_db->set_interact_code($fromUsername,$code);//显示当前活动主页面，并监听
					}
			 ...
			 */////////////////////////////////////////////////////////////////
				...
				if ($action == "room_apply_send_QR_Code"){
					...
					if (($url=img_parse($pic)) === false){
						...
						$room_name = getTempDataWithIndex($db,$fromUsername, $index);				//存入临时表中
						insert_room_record($db,$fromUsername, $room_name, $url);
						$code = "43";
					}
			 ...
			 */////////////////////////////////////////////////////////////////
				...
				if ($postObj->Event == "subscribe"){	//订阅消息
					$sender_type_picText_Title = WINDOWS::$type['welcome'];
					$content = WINDOWS::$type['wechat_main_pictxt'];			//主交互界面
					$sender_type = 'content_picText';
					$code = "0";
					$wechat_user_db->set_interact_code($fromUsername,$code);//显示当前活动主页面，并监听
				}
			...
			else if($sender_type == "content_picText"){
				if ($sender_type_picText_Title == ""){
					$sender_type_picText_Title = "深大蹭课";
				}
				$picText_list[0]['Title'] = mb_convert_encoding($sender_type_picText_Title, "UTF-8", "gbk");
				$picText_list[0]['Description'] = "";
				$picText_list[0]['PicUrl'] = "";
				$picText_list[0]['Url'] = "";

				$picText_list[1]['Title'] = mb_convert_encoding($content, "UTF-8", "gbk");
				$picText_list[1]['Description'] = "";
				$picText_list[1]['PicUrl'] = "";
				$picText_list[1]['Url'] = "";

				$picText_list[2]['Title'] = mb_convert_encoding("更多详情，敬请期待", "UTF-8", "gbk");
				$picText_list[2]['Description'] = "";
				$picText_list[2]['PicUrl'] = "";
				$picText_list[2]['Url'] = "";
				$msgObj->sendPictureMsg($fromUsername, $toUsername, $picText_list);
			}
			else{										//默认发送文本
